...
public struct EngineResult {
    ...
    public let sections: [WatchSection]
}
...
public final class Engine {
    ...
    /// Generic, non-chess watches read from `Config.watchers`.
    private let watchers = GenericJSONConnector()
    ...
    public func refresh() async -> EngineResult {
        // Chess and the generic watchers poll concurrently.
        async let chessSnapshot = chess.fetch()
        async let watcherSections = watchers.poll()

        let snapshot = await chessSnapshot
        var sections = chess.sections(from: snapshot)
        sections += await watcherSections
...
public protocol ReasoningHook {
    func process(_ sections: [WatchSection]) async -> [WatchSection]
}

public struct NoopReasoningHook: ReasoningHook {
    public init() {}
    public func process(_ sections: [WatchSection]) async -> [WatchSection] { sections }
}
